...
/* ------------------------------------------------------------------------- */
static int gt_handleEvents( GT *gt, SDL_Event *event )
{
   switch( event->type )
   {
      case SDL_QUIT:
...
int gt_procEvent( GT *gt )
{
   ...
   if( SDL_WaitEvent( &event ) )
   {
      do
      {
         result = gt_handleEvents( gt, &event );
      }
...
char *gt_dirDeleteLastSeparator( const char *path )
{
   static char result[ PATH_MAX ];
